...
```bash
# Local Databricks-backed setup
export MEMPALACE_WAREHOUSE_ID=<sql-warehouse-id>
export MEMPALACE_DATABRICKS_PROFILE=qorvo-dna01  # optional; SDK auto-auth is used otherwise
...
results = search_memories("auth decisions")
# Inject into your local model's context
```

This Databricks branch runs locally but stores memory in Unity Catalog Delta tables and Mosaic AI Vector Search.
...
`--palace <path>` is accepted for legacy compatibility but ignored. Storage is configured through Databricks env vars.

---

## Configuration

Local installs use the Databricks SDK for authentication.

```bash
pip install -e ".[dev]"

export MEMPALACE_WAREHOUSE_ID=<sql-warehouse-id>
export MEMPALACE_DATABRICKS_PROFILE=qorvo-dna01  # optional
export MEMPALACE_CATALOG=scratch                 # optional, default scratch
export MEMPALACE_SCHEMA=mitch_grabner            # optional, default mitch_grabner
export MEMPALACE_VS_ENDPOINT=mempalace_vs_endpoint
```

Identity and people-map config live in the Unity Catalog Volume at `/Volumes/<catalog>/<schema>/mempalace_config`.
...
| `databricks_backend.py` | Shared Databricks SQL + Vector Search backend |
...
| `searcher.py` | Semantic search via Mosaic AI Vector Search |
| `layers.py` | 4-layer memory stack |
| `dialect.py` | AAAK compression — 30x lossless |
| `knowledge_graph.py` | Temporal entity-relationship graph in Delta tables |
...
Databricks auth is resolved by the SDK through profile/env/app identity.
